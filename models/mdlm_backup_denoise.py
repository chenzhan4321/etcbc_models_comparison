...
class MDLMModel(BaseSequenceModel):
    ...
    def build_model(self):
        ...
        default_config = {
            ...
            'max_length': 256,
            'mask_ratio': 0.3,  # 掩码比例
            'diffusion_steps': 1,  # 扩散步数（默认1，可通过config指定）
        }
    ...
    def forward(self, input_ids: torch.Tensor, target_ids: Optional[torch.Tensor] = None,
        ...
        # 时间步编码（扩散过程的关键）
        if timestep is None:
            timestep = 0  # 默认使用第一个时间步

        # 确保timestep在有效范围内
        timestep = max(0, min(timestep, self.config['diffusion_steps'] - 1))

        # 添加时间步嵌入
        time_emb = self.time_embeddings(torch.tensor(timestep, device=x.device, dtype=torch.long))
    ...
    def compute_loss(self, input_ids: torch.Tensor, target_ids: torch.Tensor,
        ...
        # 步骤2: 添加噪声（掩码）
        noisy_input = self.add_noise(input_ids, timestep)

        # 步骤3: 前向传播（去噪）
        logits = self.forward(noisy_input, target_ids, timestep=timestep)  # [batch, seq_len, vocab_size]

        # 步骤4: 提取标签logits并计算损失
        label_logits = logits[:, :, self.label_offset:self.label_offset + self.label_vocab_size]
        ...
        label_logits = label_logits[:, :min_len, :]
        target_ids = target_ids[:, :min_len]

        # 重塑为2D
        label_logits_2d = label_logits.contiguous().view(-1, self.label_vocab_size)
        target_ids_1d = target_ids.contiguous().view(-1)
        ...
        # 只对有效位置计算损失
        valid_logits = label_logits_2d[valid_mask]
        valid_targets = target_ids_1d[valid_mask]
